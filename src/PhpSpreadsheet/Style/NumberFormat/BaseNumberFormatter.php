...
abstract class BaseNumberFormatter extends BaseFormatter
{
    ...
    protected static function formatProcessNumber($value, $format, $apply_scale = true, $useDecimalPoint = self::FORMAT_ADD_DECIMAL_OPTIONAL): string
    {
        ...
        $decimal_part = $aValue[1] ?? '';

        // Add placeholder for integer if missing
        if (((int) $integer_part > 0) && (!preg_match(self::PREG_DETECT_DIGITPLACEHOLDERS, $format_integer))) {
            $format_integer .= '#';
        }
     ...
     * @param bool $boolReverse Reverse the value
     * @param int $useDecimalPoint Add decimal point
     * @param bool $useThousands Use thousand signs
    ...
    {
        $float = (float) (substr($integer_string, 0, PHP_FLOAT_DIG) . '.' . substr($integer_string, PHP_FLOAT_DIG));
    ...
    protected static function mb_strrev($string)
    {
        ...
        return implode('', array_reverse($match[0]));
    }
     ...
     *
     * @param string $char
     * @param bool $thousand_pos The character is positioned right after a
     * thousand sign
