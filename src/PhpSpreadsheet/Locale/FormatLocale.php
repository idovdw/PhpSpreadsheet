...
abstract class FormatLocale extends CalendarData
{
    ...
    /**
     * @var bool Handling built-in formats
     */
     ...
     *     KOR fmt 55: "yyyy/mm/dd"
     */
    protected static $builtinFormatCodesList = [
        ...
        if (!empty($aLocaleCodes['codes'])) {
            ...
            foreach ($matches[1] as $pos => $code) {
                if (preg_match('/\$\-[0-9A-F]+/', $code)) {
                    ...
                    $lcid_nn = $lcid_nn >> 24;
                    $aLocaleConfig['numerals'] = LocaleLayout::$aLocaleNumerals[$lcid_nn] ?? LocaleLayout::$aLocaleNumerals[0x01];
    ...
    {
        return $this->dateSeparator ?? self::defaultDateSeparator;
    }
