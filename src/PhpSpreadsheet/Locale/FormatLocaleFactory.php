...
class FormatLocaleFactory
{
     ...
     */
    protected static $locales_translations = [
        'af' => 'af-za',
        'am' => 'am-et',
        'ar' => 'ar-sa',
        'arn' => 'arn-cl',
     ...
     * - LCID hexadecimal code, e.g. "406" (= "da-dk") or "409" (= "en-us")
     *
     * @param int|string $locale_tag_raw The (short) locale tag
     * @param int|true $default_locale The locale to be used when the LCID was not found. True will return the current user locale.
     *
     * @return null|string The locale tag based upon the LCID list (e.g. 'en_us').
     */
    public static function parseLocaleTag($locale_tag_raw, $default_locale = null)
    {
        ...
        $lcid = 0;
        if (is_int($locale_tag_raw)) {
            $lcid = $locale_tag_raw & 0xFFFF;
        } elseif (is_string($locale_tag_raw)) {
     ...
     * @param bool $try_current If the object is not found, the current locale object will be used
     *
    ...
    public static function getLocaleObjectByLCID($lcid = FormatLocale::LCID_CURRENT_LOCALE, $try_current = true)
    {
        ...
        if ($tag === false) {
            ...
            $tag = self::$lcid_locales[$lcid] ?? CurrentLocale::getLocale();
        }

        // Load the locale class
        $class_name = __NAMESPACE__ . '\\Format\\lang_' . self::getSlug($tag);
        if (class_exists($class_name)) {
            $oFormatLocale = call_user_func([$class_name, 'getInstance']);
        } else {
            if ($try_current) {
